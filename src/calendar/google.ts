...
const parse = {
  ...
  event(cal: Calendar, ev: calendar_v3.Schema$Event): Event {
    return {
      ...
      created: new Date(ev.created!),
      updated: new Date(ev.updated ?? ev.created!),
      start: new Date(ev.originalStartTime?.dateTime ?? ev.start?.dateTime!),
...
export class GoogleCalendar implements ICalendar {
  static async fetch(account: string, calendar: string) {
    ...
    const me = await user.people
      .get(
        { personFields: 'names,emailAddresses', resourceName: 'people/me' },
        {}
      )
      .then((d) => d.data);
